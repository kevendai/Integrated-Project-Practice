...
class Discharge_Predict:
    ...
    # def get_loss(self, y_test_predict):
    ...
    #     # return r2_score(y_test_reverse, y_test_predict_reverse)
    #     errors = y_test_reverse - y_test_predict_reverse
    #     return errors
    ...
    def ensamble_Discharge(self, hidden_layer_sizes=(100, 100, 100), learning_rate_init=0.001, activation='relu', solver='adam', is_save=True, is_visual=True, max_iter=3000,
                        kernel='linear', C=1.0, gamma='scale', tol=1e-3, score_func="mse"):
        """
        stack 集成学习
        ...
        model = MLPRegressor(hidden_layer_sizes=hidden_layer_sizes, max_iter=max_iter,
                             learning_rate_init=learning_rate_init, activation=activation, solver=solver)
        model.fit(self.X_train, self.y_train)
        y_train_predict1 = model.predict(self.X_train)
        y_test_predict1 = model.predict(self.X_test)
        self.y_test_predict1 = self.reverse_method(y_test_predict1, self.reverse_param1, self.reverse_param2)
        self.y_test_reverse1 = self.reverse_method(self.y_test, self.reverse_param1, self.reverse_param2)
        # self.model = model
        if is_visual:
            self._Visualization(y_train_predict1, y_test_predict1, is_save=is_save)
        loss1 = self.get_loss(model, self.X_test, self.y_test)
        results.append(loss1)
        ...
        model = SVR(kernel=kernel, C=C, gamma=gamma, max_iter=max_iter, tol=tol)
        model.fit(self.X_train, self.y_train)
        y_train_predict2 = model.predict(self.X_train)
        y_test_predict2 = model.predict(self.X_test)
        self.y_test_predict2 = self.reverse_method(y_test_predict2, self.reverse_param1, self.reverse_param2)
        self.y_test_reverse2 = self.reverse_method(self.y_test, self.reverse_param1, self.reverse_param2)
        # self.model = model
        if is_visual:
            self._Visualization(y_train_predict2, y_test_predict2, is_save=is_save)
        loss2 = self.get_loss(model, self.X_test, self.y_test)
        results.append(loss2)
        names.append(self.model_name)

        #----简单平均法
        self.model_name = 'simple-average'
        y_train_predict = (y_train_predict2 + y_train_predict1)/2
        y_test_predict = (y_test_predict2 + y_test_predict1)/2
        if is_visual:
            self._Visualization(y_train_predict, y_test_predict, is_save=is_save)
        results.append((loss1+loss2)/2)
        names.append(self.model_name)

        #--------------stack
        self.model_name = "Stack"
        level0 = list()
        level0.append(('MLP', MLPRegressor(hidden_layer_sizes=hidden_layer_sizes, max_iter=max_iter,
                             learning_rate_init=learning_rate_init, activation=activation, solver=solver)))
        level0.append(('svm', SVR(kernel=kernel, C=C, gamma=gamma, max_iter=max_iter, tol=tol)))
        level1 = LinearRegression()
        model = StackingRegressor(estimators=level0, final_estimator=level1, cv=5)
        model.fit(self.X_train, self.y_train)
        y_train_predict3 = model.predict(self.X_train)
        y_test_predict3 = model.predict(self.X_test)
        self.y_test_predict = self.reverse_method(y_test_predict3, self.reverse_param1, self.reverse_param2)
        self.y_test_reverse = self.reverse_method(self.y_test, self.reverse_param1, self.reverse_param2)
        # self.model = model
        if is_visual:
            self._Visualization(y_train_predict3, y_test_predict3, is_save=is_save)
        results.append(self.get_loss(model, self.X_test, self.y_test))
        names.append(self.model_name)
    ...
    def Grid_search_CV(self, model_name, cv=3, is_visual=True, max_iter=100, verbose=0, score_func="mse"):
        ...
        if model_name == "BP神经网络":
            ...
            model = GridSearchCV(MLPRegressor(max_iter=max_iter), param_grid, cv=cv, verbose=verbose)#cv参数是交叉验证折数的意思 verbose表示是否输出信息 0 1 2
            model.fit(self.X_train, self.y_train)
            y_train_predict = model.predict(self.X_train)
            y_test_predict = model.predict(self.X_test)
            self.y_test_predict = self.reverse_method(y_test_predict, self.reverse_param1, self.reverse_param2)
...
if __name__ == "__main__":
    ...
    #   应该是reverse_Z_score
    discharge_predict = Discharge_Predict(data, result, reverse_method=reverse_Z_score, reverse_param=(origin_mean, origin_std))
    # discharge_predict.BPNN_Discharge(learning_rate_init=0.01)
    # discharge_predict.SVM_Discharge(C=1, gamma='scale', tol=0.001)
    discharge_predict.ensamble_Discharge(learning_rate_init=0.01, C=1, gamma='scale', tol=0.001)
